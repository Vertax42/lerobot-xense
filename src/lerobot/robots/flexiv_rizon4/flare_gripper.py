...
class FlareGripper:
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(f"Error scanning sensors: {e}")
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            if self._available_sensors:
                for sn in self._available_sensors.keys():
                    self._sensors[sn] = Sensor.create(sn, mac_addr=self._mac_addr, rectify_size=self._rectify_size)
                self._logger.info(f"✅ {len(self._sensors)} tactile sensors successfully connected.")
            else:
                self._logger.warn("No tactile sensors found")
        except Exception as e:
            raise RuntimeError(f"Error connecting to Flare Gripper tactile sensors: {e}")
        ...
        try:
            ...
            if camera_id is None:
                ...
                self._logger.info(f"✅ Camera successfully connected.")
        except Exception as e:
            raise RuntimeError(f"Error connecting to Flare Gripper camera: {e}")

        try:
            # connect gripper
            self._gripper = XenseGripper.create(self._mac_addr)
            if self._gripper is not None:
                self._logger.info(f"✅ Gripper successfully connected.")
            else:
                self._logger.warn("No gripper found")
        except Exception as e:
            raise RuntimeError(f"Error connecting to Flare Gripper gripper: {e}")

        self._is_connected = True
        self._logger.info(f"✅ Flare Gripper successfully connected.")
    ...
    def register_button_callback(self, event_type: str, callback):
        if self._is_connected and self._gripper is not None:
            ...
            raise DeviceNotConnectedError(f"Flare Gripper not connected, cannot register button callback")
    
    def get_sensor(self, id: Union[int, str]) -> Sensor | None:
        ...
        if id not in self._sensors:
            self._logger.error(
                f"Sensor {id} not found, available sensors: {list(self._sensors.keys())}"
            )
            return None
