...
For that we need to define a version of the robot that takes actions in the end-effector space. The configuration typically includes:
- End-effector bounds (min/max x, y, z positions in meters)
- Maximum gripper position
- End-effector step sizes for each direction
