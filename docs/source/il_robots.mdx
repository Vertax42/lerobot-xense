...
Use the `lerobot-teleoperate` command with your robot and teleoperator configuration to start teleoperation.
...
Now you can record a dataset. To record 5 episodes and upload your dataset to the hub, use the `lerobot-record` command with your robot and teleoperator configuration:
...
lerobot-record \
    ...
    --robot.id=my_awesome_follower_arm \
    --robot.cameras="{ front: {type: opencv, index_or_path: 0, width: 1920, height: 1080, fps: 30}}" \
    --teleop.type=<your_teleop_type> \
    --teleop.port=<your_teleop_port> \
    --teleop.id=my_awesome_leader_arm \
    --display_data=true \
...
Replace `<your_robot_type>`, `<your_robot_port>`, `<your_teleop_type>`, and `<your_teleop_port>` with your actual robot and teleoperator configuration.
...
lerobot-record  \
  ...
  --robot.id=my_awesome_follower_arm \
  --display_data=false \
  --dataset.repo_id=${HF_USER}/eval_my_robot \
